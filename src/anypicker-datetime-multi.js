...
AnyPicker.prototype = $.extend(AnyPicker.prototype, {
	...
	*/
	...
	setDateInFormat: function(oInput, sType)
	{
		...
		var dDate;
		if(!$.CF.isValid(sType))
			dDate = this._getDate(new Date(oInput.iDate.y, oInput.iDate.M, oInput.iDate.d, oInput.iDate.H, oInput.iDate.m, oInput.iDate.s, oInput.iDate.ms));
		else if(sType === "START")
			dDate = this._getDate(new Date(oInput.iDate.y, oInput.iDate.M, oInput.iDate.d, 0, 0, 0, 0));
		else if(sType === "END")
			dDate = this._getDate(new Date(oInput.iDate.y, oInput.iDate.M, oInput.iDate.d, 23, 59, 59, 999));
	...
	{
		var apo = this;
		return new this._getDate(Date(dIpDate.getTime() - ((sIpTZOffset === undefined || sIpTZOffset === "" || sIpTZOffset === null) ?  -(dIpDate.getTimezoneOffset() * $.AnyPicker.extra.iMS.m) : apo._getTZOffsetInMS(sIpTZOffset))));
	},
